...
use shared_components::{
    *,
    gpp_error,
    reporter::{ CompilationError, CompilerErrorReporter },
    token::*,
};
...
impl Stage for SemanticAnalyzer {
    ...
    fn run(&mut self, ctx: &mut CompilerContext) {
        let annotated_statements = self.analyze(
            ctx.reporter.clone(),
            ctx.statements.clone().unwrap()
        );
        ctx.annotated_ast = Some(annotated_statements);
    }
}
...
impl SemanticAnalyzer {
    ...
    pub fn analyze(
        ...
        match self.get_function("main") {
            ...
            Some(f) => {
                if f.arity != 0 {
                    gpp_error!("Main function cannot takes any arguments.");
                }
            }
        }
    ...
    fn analyze_stmt(&mut self, stmt: &Statement) -> AnnotatedStatement {
        match stmt {
            ...
            Statement::NativeFunction(name, params, return_kind) => {
                self.analyze_native_function_declaration(name, params, return_kind)
            }
    ...
    fn analyze_native_function_declaration(
        &mut self,
        name: &Token,
        params: &Vec<FieldDeclaration>,
        ...
        if let TokenKind::Operator(op) = token.kind {
            match op {
                ...
                | OperatorKind::LessEqual => {
                    ...
                    if
                        left_kind.implements_archetype(&Archetype::new("str".to_string())) &&
                        right_kind.implements_archetype(&Archetype::new("str".to_string()))
                    {
                        return AnnotatedExpression::Arithmetic(
    ...
    fn define_native_function(&mut self, name: String, prototype: FunctionPrototype) {
        self.symbol_table.native_functions.insert(name, prototype);
    }
}
